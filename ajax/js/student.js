    ...
    function displaySubject() {
        $.ajax({
            ...
            success: function (data) {
                let content = ''
                for (let i = 0; i < data.length; i++) {
                    content += `<input type="checkbox" name="subject" value="${data[i].idSubject}">${data[i].name}`
                }
                document.getElementById("checkbox").innerHTML = content;
